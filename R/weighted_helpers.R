...
wt.rg.S <- function(
  ...
  if (scheme == "fh") {
    ...
    wt <- 1 / mS
  } else  if (scheme == "custom_time" && (is.null(scheme_params$tpoints) || length(scheme_params$tpoints) != length(S.pool) || is.null(scheme_params$t.tau) || is.null(scheme_params$w0.tau) || is.null(scheme_params$w1.tau))) {
    stop("For custom_time weights, provide tpoints (time points) of same length as S, t.tau (cutoff time), w0.tau, and w1.tau in scheme_params.")
...
validate_scheme_params <- function(scheme, scheme_params, S.pool, tpoints) {
  ...
  if (scheme == "custom_time" && (is.null(scheme_params$tpoints) || length(scheme_params$tpoints) != length(S.pool) || is.null(scheme_params$t.tau) || is.null(scheme_params$w0.tau) || is.null(scheme_params$w1.tau))) {
    stop("For custom_time weights, provide tpoints (time points) of same length as S, t.tau (cutoff time), w0.tau, and w1.tau in scheme_params.")
  }
}

validate_scheme_params_old <- function(scheme, scheme_params, S.pool, tpoints) {
  if (scheme == "fh" && (is.null(scheme_params$rho) || is.null(scheme_params$gamma))) {
    stop("For Fleming-Harrington weights, specify both rho and gamma in scheme_params.")
  }
  if (scheme == "schemper" && (is.null(scheme_params$Scensor) || length(scheme_params$Scensor) != length(S.pool))) {
