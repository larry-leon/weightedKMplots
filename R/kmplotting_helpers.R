...
#' KM plotting
#' @export
KM_plot_2sample_weighted_counting <- function(dfcount, show.cox = FALSE, cox.cex = 0.725, show.logrank = FALSE,
  ...
  if (check.KM) {
    ...
    check_km_curve(S0.KM, "Group 0")
    check_km_curve(S1.KM, "Group 1")
  }
  plot_km_curves_counting(at.points, S0.KM, idx0, idv0, S1.KM, idx1, idv1, col.0, col.1, ltys, lwds, Xlab, Ylab,
  ...
  if(show.med){
    medians_df <- dfcount$quantile_results

    medians_df$label <- paste0(
      format(medians_df$quantile, digits=med.digits), " [", format(medians_df$lower,digits=med.digits), ", ", format(medians_df$upper,digits=med.digits), "]"
    )
