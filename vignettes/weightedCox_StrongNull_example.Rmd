...
#load("/media/larryleon/My Projects/GitHub/weightedKMplots/vignettes/results/wCoxsims1_Linux_20k.RData")
...
dfa <- analysis_data
dfa$treat <- ifelse(dfa$treatment=="experimental",1,0)
dfcount <- get_dfcounting(df=dfa, tte.name = "tte", event.name = "event", treat.name = "treat", arms = "treatment", by.risk= 3, rho=0, gamma=1)
...
temp <- wlr_cumulative(dfcounting = dfcount, scheme_params = list(rho = 0, gamma = 1), scheme = "fh")

with(temp, plot(time, z_cumulative, xlab="time", ylab="cumulative log-rank Z", type="s", ylim=c(ymin,ymax))
)
z01 <- temp$z_cumulative

rm("temp")
abline(h = qnorm(0.975), lwd=2, col="red")
title(main="FH(0,1)")
temp <- wlr_cumulative(dfcounting = dfcount, scheme_params = list(rho = 0, gamma = 0), scheme = "fh")
with(temp, plot(time, z_cumulative, xlab="time", ylab="cumulative log-rank Z", type="s", ylim=c(ymin,ymax))
     )
rm("temp")
abline(h = qnorm(0.975), lwd=2, col="red")
title(main="FH(0,0)")
temp <- wlr_cumulative(dfcounting = dfcount, scheme = "fh_exp2")
with(temp, plot(time, z_cumulative, xlab="time", ylab="cumulative log-rank Z", type="s", ylim=c(ymin,ymax)))
abline(h = qnorm(0.975), lwd=2, col="red")

z1 <- temp$z_cumulative

title(main="FH_exp2")
rm("temp")
temp <- wlr_cumulative(dfcounting = dfcount, scheme_params = list(mb_tstar = 12), scheme = "MB")
with(temp, plot(time, z_cumulative, xlab="time", ylab="cumulative log-rank Z", type="s", ylim=c(ymin,ymax))
     )
z2 <- temp$z_cumulative
abline(h = qnorm(0.975), lwd=2, col="red")
title(main="MB(12)")
