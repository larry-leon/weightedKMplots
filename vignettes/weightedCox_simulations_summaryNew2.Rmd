...
source("R/kmplotting_helpers.R")
source("R/df_counting.R")
source("R/df_counting_helpers_revised.R")
source("R/km_wlr_calculations_helpers.R")
source("R/weightedCox.R")
...
#load("~/Library/CloudStorage/OneDrive-MerckSharp&DohmeLLC/documents/GitHub/weightedKMplots/vignettes/results/wCoxsims1_linux_50k.RData")

#load("/media/larryleon/My Projects/GitHub/weightedKMplots/vignettes/results/wCoxsims1_linux_n400_5k.RData")
...
# Add any remaining columns not in z_main
#z_order2 <- c(z_order2, setdiff(z_cols_nondebiased, z_order2))
# NOTE: here we only want to view stats in z_main above
# Set factor levels for facet order
#long_df2$z_stat <- factor(long_df2$z_stat, levels = z_order2)
...
results |> group_by(Scenario) |> 
  ...
  gt() |> fmt_number(columns = 2:12, decimals = 3)  
