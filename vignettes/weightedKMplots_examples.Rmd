...
This will go with Cox stuff (just keeping here, for now)

$$
U(\beta) = \sum_{\text{event times}} \sum_{l=1}^{d} \left[ x_{i_l} - \frac{ \sum_{j \in R} x_j \exp(x_j \beta) - \frac{l-1}{d} \sum_{j \in D} x_j \exp(x_j \beta) }{ \sum_{j \in R} \exp(x_j \beta) - \frac{l-1}{d} \sum_{j \in D} \exp(x_j \beta) } \right]
$$
