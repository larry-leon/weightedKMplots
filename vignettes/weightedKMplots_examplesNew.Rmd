...
GBSG - ITT Analysis
```{r}
dfcount_gbsg <- df_counting(df=df_gbsg, tte.name=tte.name, event.name=event.name, treat.name=treat.name, arms=arms, by.risk = 12, scheme = "fh", scheme_params = list(rho = 0, gamma =0), lr.digits = 4, cox.digits =3)
```
...
temp <- plotKM.band_subgroups(
    ...
    tte.name = "tte", treat.name = "treat", event.name = "event", draws.band = draws_se, qtau = 0.025, show_resamples = FALSE
  )
...
# RMST
tpoints <- temp$xpoints 
dhat <- temp$mean
rmst_tau <- temp$fit_itt$max_tau

rmst <- LS.int(dhat, tpoints, L = Inf)

#rmst_check <- rmst2(time = dfa$tte, status = dfa$event, arm = dfa$treat, tau = rmst_tau)
# Extract RMST for arm1-arm0
#cat("Mine vs rmst2", c(rmst,rmst_check$unadjusted.result[1,1]),"\n")

# Cumulative version
rmst_time <- unlist(lapply(as.list(tpoints), mu.L, S = dhat, t = tpoints))

dhat_star_mat <- temp$fit_itt$dhat_star
rmst_time_stars <- dhat_star_mat
ndraws <- ncol(dhat_star_mat)
for(dd in 1:ndraws){
dhat_star <- dhat_star_mat[,dd]
# Cumulative centered version
rmst_time_stars[,dd] <- unlist(lapply(as.list(tpoints), mu.L, S = dhat_star, t = tpoints))
}
sig2_rmst_time <- apply(rmst_time_stars, 1, var, na.rm=TRUE)
...
# Normalized resamples
at_points <- temp$fit_itt$at_points
risk.points <- temp$fit_itt$risk.points

fit_draws <- KM_diff(
    df = dfa, tte.name = "tte", event.name = "event", treat.name = "treat", weight.name = NULL,
...
rmst_time_stars2 <- dhat_star_mat2
ndraws <- ncol(dhat_star_mat2)

for(dd in 1:ndraws){
dhat_star <- dhat_star_mat2[,dd]
# Cumulative centered version
rmst_time_stars2[,dd] <- unlist(lapply(as.list(tpoints), mu.L, S = dhat_star, t = tpoints))
}

# Standardized
rmst_time_star3 <- rmst_time_stars2 / sqrt(sig2_rmst_time)


# simultaneous band
sups <- apply(abs(rmst_time_star3), 2, max, na.rm = TRUE)
c_alpha_band <- quantile(sups,c(0.95))

# Simultaneous bands
rmst_time_SBlower <- rmst_time - c_alpha_band * sqrt(sig2_rmst_time)
rmst_time_SBupper <- rmst_time + c_alpha_band * sqrt(sig2_rmst_time)

x <- tpoints
mean.value <- rmst_time
l0_pw <- rmst_time_lower
u0_pw <- rmst_time_upper 
l0_sb <- rmst_time_SBlower
u0_sb <- rmst_time_SBupper
...
ymax <- max(u0_sb)
 ...
 plot(
    x[order(x)], mean.value[order(x)], type = "n", xlab = "time", lty = 1,
...
Zero/one weighting (0 prior to 25 months, 1 thereafter using custom_time scheme)
```{r}

dfcount <- df_counting(df=df_gbsg, tte.name=tte.name, event.name=event.name, treat.name=treat.name, arms=arms, by.risk = 12, 
scheme = "custom_time", scheme_params = list(t.tau = 25, w0.tau = 0, w1.tau = 1), lr.digits = 4, cox.digits =3)

g <- plot_weight_schemes(dfcount, transform_fh = FALSE, rescheme_fhexp2 = TRUE)
print(g)
...
temp <- wlr_dhat_estimates(dfcounting = dfcount_gbsg, scheme_params = list(rho = 0, gamma = 0), scheme = "fh")
