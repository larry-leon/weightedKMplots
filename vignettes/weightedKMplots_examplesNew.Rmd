...
temp <- plotKM.band_subgroups(
    ...
    tte.name = tte.name, treat.name = treat.name, event.name = event.name , weight.name = weight.name, 
    draws.band = draws_se, qtau = 0.025, show_resamples = FALSE
  )
...
get_bands <- cumulative_rmst_bands(df = dfa, fit = temp$fit_itt, 
tte.name = tte.name, event.name = event.name, treat.name = treat.name , weight.name = weight.name, 
draws_sb = draws_sb, xlab="months", rmst_max_cex = 0.75)
